...
public partial class MainPage : ContentPage
{
    ...
    private async void CalcItem(object sender, EventArgs e)
    {
        ...
        var selection = ShopView.SelectedItem as Products;

        //Get selection and Set TXT content off selections 
        sumID.Text = selection.Id.ToString();
        sumName.Text = selection.Product.ToString();
        sumCalcCode.Text = selection.Product.ToString();
        

        var costP = Convert.ToInt32(selection.Price);
        ...
        await DisplayAlert($"Id: {selection.Id} Code: {selection.Code}", $"You selected: {selection.Product}", "Ok");
